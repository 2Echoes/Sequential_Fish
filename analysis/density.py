...
def multichannel_clustering(
    ...
    Aggregates results in resulting pd.DataFrame with keys :
        ...
        Axis 0 (Index) :
        ----------------
            general_cluster_id : int, unique identifier for multichannel clusters.
        
        Axis 1 (Columns) :
        ------------------
            ('spot_id','count') : number of single molecules in cluster
            ('target','nunique')
    ...
    master_spots = _merge_data_for_general_DBSCAN(Acquisition, Detection, Spots, Gene_map, no_filtering=no_filtering)
    clustered_spots = _run_DBSCAN(master_spots, voxel_size=voxel_size, min_number_cluster=nb_min_spots)
...
def _merge_data_for_general_DBSCAN(
        Acquisition : pd.DataFrame, 
        Detection : pd.DataFrame, 
        Spots : pd.DataFrame, 
        Gene_map : pd.DataFrame,
...
def _run_DBSCAN(Spots : pd.DataFrame, voxel_size : tuple, min_number_cluster = 4) :
    """
...
def _merge_DBSCAN_results(Spots_main : pd.DataFrame, Spots_clustered : pd.DataFrame) :
    check_len = len(Spots) #No filtering/duplication expected.
